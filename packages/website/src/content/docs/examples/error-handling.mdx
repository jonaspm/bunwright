...
description: Fail-fast chains, TimeoutError handling, soft exists().
---
...
- **Fail-fast chains** — if `click` throws, `waitForURL` is skipped
- **`instanceof TimeoutError`** — branch on the specific error type
- **`exists()`** — soft check that returns `boolean` instead of throwing
